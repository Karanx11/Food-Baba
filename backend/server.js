...
const PROMPT = [
  ...
  'drink in the photo and report its nutrition. Items come in two kinds;',
  'handle each the right way.',
  '',
  'A) PACKAGED or BRANDED products — a wrapper, box, bottle, can, tetra pack,',
  '   or any visible "Nutrition Facts" / nutritional-information panel:',
  '   - Read the product. Set brand to the maker (e.g. "Lay\'s", "Amul",',
  '     "Maggi") and put the full product in name (e.g. "Lay\'s Classic',
  '     Salted", "Amul Taaza Toned Milk").',
  '   - If a nutrition label is legible in the photo, READ the printed',
  '     numbers. Use the label\'s serving size for gramsPerServing and the',
  '     label\'s per-serving calories, protein, carbs, fat, sugar, fibre and',
  '     sodium. Do NOT estimate when the label is readable; transcribe it.',
  '     If the label lists per 100 g/ml only, scale to the stated serving.',
  '   - If no label is legible, use the known published values for that exact',
  '     branded product.',
  '   - servings: default 1 (one labelled serving). Go above 1 only when',
  '     several packs or units are clearly shown.',
  '   - Add "from label" (or "packaged") to notes.',
  '',
  'B) COOKED, PLATED or LOOSE foods with no packaging: estimate from standard',
  '   food-composition data (USDA FoodData Central and the Indian Food',
  '   Composition Tables). Be realistic and typical, never generous; use',
  '   median values when unsure, and do not inflate calories, protein or',
  '   portion size.',
  ...
  '',
  'For EVERY item, report per ONE serving:',
  '- servingLabel: a natural unit, e.g. "1 pack", "1 bottle", "1 plate",',
  '  "1 katori", "2 rotis", "1 glass".',
  '- gramsPerServing: the weight (or ml for drinks) of ONE such serving.',
  '- servings: how many servings are shown. Use 1 unless clearly more.',
  '- calories, proteinG, carbsG, fatG, sugarG, fiberG per ONE serving;',
  '  sodiumMg per serving in milligrams.',
  '- confidence: 0..1 (higher when read from a label).',
  '- notes: a short phrase under 12 words — label source or notable',
  '  nutrients; empty string if nothing stands out.',
  '',
  'Keep macros consistent with calories: per gram, calories are about',
  '4*protein + 4*carbs + 9*fat. Rice, breads and most mixed dishes are mainly',
  'carbohydrate, not protein. Sanity check before answering: a normal plate of',
  'biryani is about 450-650 kcal with 20-30 g protein, not more. If the photo',
  'has no food or drink, return an empty foods array.',
].join('\n');
